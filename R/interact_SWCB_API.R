...
#'
#' @param wallet Select the wallet (with appropriate permissions)
...
interact_AT_API <- function(wallet = c("BTC", "USD", "DOGE"),
                            method = c("GET", "POST"),
                            reqPath,
                            query = "",
                            body = ""){
  ...
  auth <- rcbatapi::authentication_int(wallet = wallet,
                                       method = method,
